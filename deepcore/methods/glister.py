...
class Glister(EarlyTrain):
    ...
    def calc_gradient(self, index=None, val=False, record_val_detail=False):
        ...
        # gradients -> List len=40， （40，128，5132）
    ...
    def finish_run(self):
        ...
        if self.balance:
            ...
            for c in range(self.num_classes):
                ...
                # print("44=====> :",self.train_grads[1].shape, self.n_train, self.n_val)
                # print("55=====> :",self.val_grads.view(-1, 1).detach().cpu().numpy().flatten().shape)
                # print("66=====> :",self.val_grads.view(-1, 1).detach().cpu().numpy().flatten())
                # print("66=====> :",torch.matmul(self.train_grads[1],
                #                                              self.val_grads.view(-1, 1)).detach().cpu().numpy().
                #                                              flatten())
                selection_result = np.append(selection_result, c_selection_result)
            print("========> selection_result_shape", selection_result.shape)
            with open("./select_glister.txt", "w") as f:
                f.write(str(tuple(selection_result))+str("\n"))
                f.close()
            print("=======> : save over")
        else:

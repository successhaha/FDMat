...
class GraNd(EarlyTrain):
    ...
    def finish_run(self):  # 训练指定的epoch后开始进行数据选择
        ...
        for i, (input, targets) in enumerate(batch_loader): # 从总的batch中加载
            self.model_optimizer.zero_grad()
            outputs = self.model(input.to(self.args.device))
            loss = self.criterion(outputs.requires_grad_(True),
                                  targets.to(self.args.device)).sum()
            # print("test ===> ", self.model.embedding_recorder.embedding) # 全连接后展平的特征（128，512）
            ...
            input （80，100）
            outputs （80， 100）
            bias_parameters_grads.shape （128，100）
            self.model.embedding_recorder.embedding （128， 100，512）
            self.norm_matrix (50000, 5)
            self.model.embedding_recorder.embedding-->torch.size([128, 512])
            """

            with torch.no_grad():
                bias_parameters_grads = torch.autograd.grad(loss, outputs)[0]
                # print("=> bias_parameters_grads: ", bias_parameters_grads.shape, 
                #       self.model.embedding_recorder.embedding.view(batch_num, 1, embedding_dim).repeat(1,
                #                              self.args.num_classes, 1).shape,
                #                              bias_parameters_grads.view(batch_num, self.args.num_classes, 1).repeat(1, 1, embedding_dim).shape)
                # print("=> test", self.norm_matrix[i * self.args.selection_batch:min((i + 1) * self.args.selection_batch, sample_num),
                # self.cur_repeat].shape, self.args.selection_batch)
                # print("=> TEST 1 ", torch.cat([bias_parameters_grads, (
                #         self.model.embedding_recorder.embedding.view(batch_num, 1, embedding_dim).repeat(1,
                #                              self.args.num_classes, 1) * bias_parameters_grads.view(
                #                              batch_num, self.args.num_classes, 1).repeat(1, 1, embedding_dim)).
                #                              view(batch_num, -1)], dim=1).shape)
                # print("test ===> ", self.model.embedding_recorder.embedding)
                ...
                # print("===> 6 " , self.norm_matrix.shape)
    ...
    def select(self, **kwargs):
        ...
        # print("norm_matrix : ", self.norm_matrix.shape)
        ...
        if not self.balance:
            ...
            for c in range(self.num_classes):
                ...
                print("____________indices",top_examples,"   ", self.norm_mean)
